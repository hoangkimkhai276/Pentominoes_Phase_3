 ...
 */
public class Knapsack2 implements Variables {
	...
	protected boolean[] occupied_cubes;
	
	public Knapsack2(int length, int width, int height) {
		this.shape = new Cube(length, width, height, Point3D.ZERO);
		parcels = new ArrayList<Parcel>();
		sorted = SortState.NONE;
		occupied_cubes = new boolean[length * width * height]; // empty array
	}
	public Knapsack2(double length, double width, double height) {
	...
	private void setBit(int x, int y, int z, boolean set) {
		occupied_cubes[z * getWidth() * getLength() + y * getLength() + x] = set;
	}
	public boolean isOccupied(int x, int y, int z) {
		return occupied_cubes[z * getWidth() * getLength() + y * getLength() + x];
	}
